...
async function deployEthereumContracts() {
  ...
  if (balance.lt(ethers.utils.parseEther("0.1"))) {
    console.warn(
      "⚠️ Low balance! You may need more testnet ETH from https://sepoliafaucet.com/"
    );
  }
  ...
  try {
    ...
    const deploymentPath = path.join(
      __dirname,
      "../deployed_contracts/ethereum-sepolia.json"
    );
...
async function deployNearContracts() {
  ...
  if (!CONFIG.near.accountId || !CONFIG.near.privateKey) {
    throw new Error(
      "NEAR_ACCOUNT_ID and NEAR_PRIVATE_KEY environment variables are required"
    );
  }

  try {
    // Check if WASM file exists
    const wasmPath = path.join(__dirname, "../near/contracts/target/wasm32-unknown-unknown/release/fusion_near_escrow.wasm");
    if (!fs.existsSync(wasmPath)) {
      throw new Error(`WASM file not found at ${wasmPath}. Run 'cd near/contracts && cargo build --target wasm32-unknown-unknown --release' first.`);
    }

    console.log("📦 WASM contract found, ready for deployment");
    console.log("🔧 To deploy to Near testnet, run:");
    console.log(`   near-cli-rs contract deploy ${CONFIG.near.accountId} use-file ${wasmPath} without-init-call network-config testnet sign-with-keychain send`);
    console.log("\n⚠️  AUTOMATED DEPLOYMENT COMING SOON");
    console.log("For now, use manual deployment with near-cli-rs commands above");
    
    // Create deployment record for manual deployment
    const contractAccount = `escrow-dst-${Date.now()}.${CONFIG.near.accountId}`;
    const deploymentInfo = {
      network: "testnet",
      timestamp: new Date().toISOString(),
      deployer: CONFIG.near.accountId,
      contracts: {
        escrowDst: contractAccount,
      },
      wasmPath: wasmPath,
      deploymentStatus: "manual_required",
      instructions: [
        "1. Install near-cli-rs: https://github.com/near/near-cli-rs",
        "2. Login: near-cli-rs account import-account using-web-wallet network-config testnet",
        `3. Deploy: near-cli-rs contract deploy ${contractAccount} use-file ${wasmPath} without-init-call network-config testnet sign-with-keychain send`
      ],
      explorerUrls: {
        escrowDst: `https://testnet.nearblocks.io/account/${contractAccount}`,
      },
    };

    const deploymentPath = path.join(
      __dirname,
      "../deployed_contracts/near-testnet.json"
    );
    fs.mkdirSync(path.dirname(deploymentPath), { recursive: true });
    fs.writeFileSync(deploymentPath, JSON.stringify(deploymentInfo, null, 2));

    console.log("✅ Near deployment configuration completed!");
    console.log(`📄 Deployment info saved to: ${deploymentPath}`);
...
async function main() {
  try {
    ...
    console.log("\n🔗 Explorer Links:");
    console.log(
      `   Sepolia: https://sepolia.etherscan.io/address/${ethereumContracts.escrowFactory}`
    );
    console.log(
      `   Near: https://testnet.nearblocks.io/accounts/${nearContracts.escrowFactory}`
    );
    console.log("\n📱 Demo Ready:");
